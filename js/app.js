...
function renderChecklist() {
  ...
  ul.innerHTML = list.items.map((item, i) => {
    ...
    return `<li class="checklist-item ${done ? 'done' : ''} ${current ? 'current' : ''}">
      ...
      <button class="item-info-btn" data-phase="${phase}" data-idx="${i}" title="Why & Show Me">â“˜</button>
    </li>`;
  }).join('');
...
function toggleItem(idx) {
  ...
  if (allDone) {
    ...
    document.getElementById('complete-time').textContent = `${CHECKLISTS[phase].label} complete â€” ${new Date().toLocaleTimeString()}`;
  } else {
    ...
    if (item.tipType === 'acronym' && item.acronym) {
      tipContent.innerHTML = `
        ...
        <div class="info-tip" style="margin-top:10px">
          <span class="info-tip-icon">ðŸ’¡</span>
          <span>${item.tip}</span>
        </div>`;
    } else {
      tipContent.innerHTML = `<div class="info-tip"><span class="info-tip-icon">ðŸ’¡</span><span>${item.tip}</span></div>`;
    }
  } else {
  ...
  if (s.rule) {
    ruleEl.innerHTML = `
      <span class="rule-badge ${s.rule.repeats ? 'repeats' : 'no-repeats'}">
        ${s.rule.repeats ? 'â†© Ends with airport name' : 'âœ• Does not repeat airport name'}
      </span>
...
function addWord(el, word) {
  // Legacy â€” Radio tab now uses delegated handler. Kept for safety.
  if (el.classList.contains('used')) {
    const key = el.dataset.key;
...
function checkRadioCall() {
  ...
  if (isCorrect) {
    document.getElementById('feedback-header').textContent = 'âœ“ Correct sequence';
    document.getElementById('feedback-ideal').textContent = s.ideal;
    let noteText = s.note;
    if (usedDistractors.length > 0) {
      noteText += '\n\nâš  Trap chips you avoided â€” good:\n' +
        usedDistractors.map(d => `â€¢ "${d.text}" â€” ${d.why}`).join('\n');
    }
    document.getElementById('feedback-note').textContent = noteText;
  } else {
    document.getElementById('feedback-header').textContent = 'âœ— Not quite';
    document.getElementById('feedback-ideal').textContent = '';

    let noteHtml = '';
    if (usedDistractors.length > 0) {
      noteHtml += 'âš  Trap chips you included:<br>' +
        usedDistractors.map(d => `â€¢ "${d.text}" â€” ${d.why}`).join('<br>') + '<br><br>';
    }
    noteHtml += `<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:4px">
      <button class="btn btn-sm" onclick="retryRadioCall()">â†© Try Again</button>
...
function revealRadioCall() {
  const s = RADIO_SCENARIOS[state.radio.scenarioIdx];
  document.getElementById('feedback-header').textContent = 'âœ— Ideal call';
  document.getElementById('feedback-ideal').textContent = s.ideal;
...
function answerEmergency(optionIdx, el) {
  ...
  hdr.textContent = isCorrect ? 'âœ“ Correct' : 'âœ— Incorrect';
...
// â”€â”€ AIRPORT DATABASE (bundled â€” works offline & file://) â”€â”€
  ...
  if (ap) {
    ...
    result.innerHTML = `
      <div class="proc-airport-result">
        ...
        <div class="proc-airport-meta">${icao} Â· Elev ${elev} ft MSL${notes ? ' Â· ' + notes : ''}</div>
      </div>`;
    manual.style.display = 'none';
  } else {
    result.innerHTML = `<div class="proc-airport-err">âš  "${icao}" not in database â€” enter values below.</div>`;
...
// â”€â”€ PROCEDURE DEFINITIONS â”€â”€
// Step types:
//   config  â€” cockpit panel control sliders/chips
//   radio   â€” word-chip call builder
//   choice  â€” multiple choice (knowledge/judgment only)
//   order   â€” drag-to-order (coming soon)
...
function buildPatternLanding(ap) {
  ...
  const fuelCorrectLabel = isCherokee ? 'Proper (fuller) tank â€” Cherokee has no BOTH position' : 'BOTH â€” draws from both tanks simultaneously';
  const fuelWrongLabel = isCherokee ? 'Select the fuller tank. Cherokee has no BOTH â€” must choose L or R.' : 'Select BOTH for maximum fuel availability and go-around power.';
  const carbHeatLabel = isCherokee ? 'Carb Heat' : 'Carb Heat';
  const hasFuelPump = isCherokee;
  const baseFlaps = isCherokee ? '25Â°' : '20Â°';
  const finalFlaps = isCherokee ? '25Â°' : '30Â°';
  const finalFlapsOptions = isCherokee ? ['0Â°', '10Â°', '25Â°', '40Â°'] : ['0Â°', '10Â°', '20Â°', '30Â°'];
  const baseFlapsOptions = isCherokee ? ['0Â°', '10Â°', '25Â°', '40Â°'] : ['0Â°', '10Â°', '20Â°', '30Â°'];
  ...
  return {
    title: `Pattern Entry & Landing â€” ${acName}`,
    steps: [

      // â”€â”€ 1. ALTITUDE â€” config
      {
        type: 'config',
        phase: 'Inbound / Setup',
        ...
        controls: [
          {
            ...
            correct: tpaMSL,
            tolerance: 50,
            correctLabel: `${icao} pattern is ${tpaMSL} ft MSL â€” standard 1,000 ft AGL above the ${elev} ft field elevation`,
            wrongLabel: `${icao} pattern altitude is ${tpaMSL} ft MSL (field elev ${elev} + 1,000 AGL). Arrive established before turning downwind.`,
          }
        ],
        feedback: 'Standard traffic pattern altitude is 1,000 ft AGL â€” always add that to field elevation to get your MSL target. Arrive at pattern altitude before entering downwind; being high or low disrupts your timing and sight picture for the whole approach.',
        tip: { title: 'Pattern entry', text: 'Enter the pattern at 45Â° to the downwind leg, abeam midfield. This lets you see traffic already in the pattern â€” the FAA-recommended entry. Don\'t barrel straight in to the base or final legs.' }
      },

      // â”€â”€ 2. DOWNWIND RADIO â€” radio
      {
        type: 'radio',
        phase: 'Downwind â€” Radio',
        ...
        distractors: [
          ...
          { text: 'any traffic please advise', why: 'Non-standard phraseology â€” discouraged by the FAA. It clutters the frequency without adding useful information.' },
          { text: 'over', why: '"Over" is not used in aviation radio calls. It\'s a civilian/military misconception â€” just say what you need to say and release the mic.' },
        ],
        feedback: 'Five pieces every CTAF call: airport + "traffic" â†’ who you are â†’ where you are + runway â†’ airport name again. The bookend airport name confirms other pilots are on the right frequency.',
        tip: { title: 'Every pattern leg', text: `Make a call on downwind, base, and final. At controlled fields, tower handles sequencing â€” but at ${icao} you\'re all each other has.` }
      },

      // â”€â”€ 3. GUMPS â€” choice (knowledge, no cheating possible â€” all plausible)
      {
        type: 'choice',
        phase: 'Downwind â€” GUMPS',
        ...
        options: [
          { text: 'Primer â€” in and locked', correct: true,  why: 'Primer is a START flow item, not GUMPS. GUMPS = Gas Â· Undercarriage Â· Mixture Â· Prop Â· Seatbelts.' },
          { text: 'Gas â€” fuel selector BOTH', correct: false, why: '' },
          { text: 'Mixture â€” rich', correct: false, why: '' },
          { text: 'Seatbelts â€” secure', correct: false, why: '' },
        ],
        feedback: 'GUMPS = Gas (BOTH) Â· Undercarriage (down/confirmed) Â· Mixture (rich) Â· Prop (full forward) Â· Seatbelts (secure). Primer is a start-up item, not a landing flow item.',
        tip: { title: 'C172 GUMPS in practice', text: 'On a fixed-gear fixed-pitch C172, Undercarriage and Prop are non-events â€” you still say them to build the habit for complex aircraft later. Run it every downwind, every time, out loud.' }
      },

      // â”€â”€ 4. ABEAM NUMBERS CONFIG â€” config (the main event)
      {
        type: 'config',
        phase: 'Downwind â€” Abeam the Numbers',
        ...
        controls: [
          {
            ...
            correct: abeamPower,
            tolerance: 100,
            correctLabel: `~${abeamPower} RPM â€” reduces speed below Vfe so flaps can be extended`,
          ...
          },
          {
            ...
            default: '0Â°',
            correct: '10Â°',
            correctLabel: `10Â° â€” first notch once below ${vfe} KIAS (Vfe)`,
            wrongLabel: `Flaps 10Â° abeam the numbers, after speed is below ${vfe} KIAS. Not full flaps yet â€” that's for final.`,
          },
          {
            ...
            correct: abeamSpeed,
            tolerance: 5,
            correctLabel: `~${abeamSpeed} KIAS â€” stabilized after first flap extension, trimmed`,
            wrongLabel: `Target ~${abeamSpeed} KIAS after flaps 10Â°. Trim to hold it â€” don't hold back pressure all the way around.`,
          },
          {
            ...
            default: 'OFF',
            correct: 'ON',
            correctLabel: 'ON â€” reduced power = susceptibility to carb ice',
          ...
          ...(hasFuelPump ? [{
            ...
            default: 'OFF',
            correct: 'ON',
            correctLabel: 'ON â€” electric pump provides backup fuel pressure for approach and go-around',
            wrongLabel: 'Fuel pump ON for approach. Provides backup pressure if the engine-driven pump falters at low power.',
          }] : []),
        ],
        feedback: `Abeam sequence: power to ~${abeamPower} â†’ hold altitude, let speed bleed â†’ below ${vfe} KIAS add flaps 10Â° â†’ trim for ${abeamSpeed} KIAS â†’ carb heat on${hasFuelPump ? ' â†’ fuel pump ON' : ''} â†’ begin descent.`,
        tip: { title: 'The order matters', text: `Power FIRST to bleed speed, THEN flaps. Adding flaps above Vfe (${vfe} KIAS on ${acName}) can damage the flap structure. Once configured, trim hands-off â€” don't muscle it around the pattern.` }
      },

      // â”€â”€ 5. BASE TURN â€” choice (judgment, distractors are real mistakes)
      {
        type: 'choice',
        phase: 'Base Turn',
        ...
        options: [
          { text: 'When the runway threshold is ~45Â° behind your wingtip', correct: true, why: '' },
          { text: 'When you are directly abeam (90Â°) the runway threshold', correct: false, why: 'Turning at 90Â° puts you on a very short base â€” you\'ll be high and fast turning final.' },
          { text: 'When the runway numbers are no longer visible through your window', correct: false, why: 'This is too late â€” you\'ll overshoot final or need a steep turn to line up.' },
        ...
        feedback: '~45Â° behind the wing is the standard reference. Watch for the runway threshold drifting to your 5 o\'clock â€” that\'s your turn cue. Wind shifts this: headwind on final = turn sooner; tailwind on downwind = extend further.',
        tip: { title: 'High on final?', text: 'If you consistently arrive high on final, the fix is usually a longer downwind before turning base â€” not adding flaps early. Flaps fix configuration, not geometry.' }
      },

      // â”€â”€ 6. BASE CONFIG â€” config
      {
        type: 'config',
        phase: 'Base Leg',
        ...
        controls: [
          {
            ...
            default: '10Â°',
            correct: baseFlaps,
            correctLabel: `${baseFlaps} â€” steepens descent, slows further for final`,
            wrongLabel: `Flaps ${baseFlaps} on base. 10Â° was abeam the numbers; full flaps added on final once runway is made.`,
          },
          {
            id: 'speed', label: 'Target Airspeed', type: 'slider',
            ...
            correct: baseSpeed,
            tolerance: 5,
            correctLabel: `~${baseSpeed} KIAS â€” margin above stall, time to stabilize`,
            wrongLabel: `Target ~${baseSpeed} KIAS on base. Faster = hard to slow for final; slower = too little margin.`,
          },
        ],
        feedback: `Base: flaps ${baseFlaps}, ~${baseSpeed} KIAS. Descending to intercept final. Keep an eye on the runway â€” if it's moving toward your nose you're overshooting; if moving away you're undershooting.`,
        tip: { title: 'Base radio call', text: `"${icao} traffic, Skyhawk Four Five Two One Golf, left base runway ${rwy}, ${icao}." Make it before the turn or early in the turn â€” other pilots on long final need to hear it.` }
      },

      // â”€â”€ 7. BASE RADIO â€” radio
      {
        type: 'radio',
        phase: 'Base â€” Radio',
        ...
        distractors: [
          { text: 'turning base', why: 'Announce the leg you\'re on, not the maneuver. Say "left base" once established â€” not "turning base" mid-turn.' },
          ...
          { text: 'souls on board two', why: 'Souls on board is emergency phraseology only â€” used during a Mayday call. Never in normal pattern calls.' },
        ],
        feedback: 'Same structure as downwind: airport â†’ callsign â†’ leg + runway â†’ airport. Keep it punchy â€” other traffic needs to process it fast.',
      ...
      // â”€â”€ 8. FINAL CONFIG â€” config
      {
        type: 'config',
        phase: 'Final â€” Configuration',
        ...
        controls: [
          {
            ...
            default: baseFlaps,
            correct: finalFlaps,
            correctLabel: `${finalFlaps} â€” full flaps for landing`,
          ...
          },
          {
            ...
            correct: shortFinal,
            tolerance: 5,
            correctLabel: `${shortFinal}â€“${approach} KIAS â€” Vref for ${acName} with full flaps`,
            wrongLabel: `Target ${shortFinal}â€“${approach} KIAS on final. Add half the gust factor in gusty conditions.`,
          },
          {
            id: 'carbheat', label: 'Carb Heat', type: 'chips',
            options: ['OFF', 'ON'],
            default: 'ON',
            correct: 'ON',
            correctLabel: 'ON â€” keep it on throughout approach',
          ...
          ...(hasFuelPump ? [{
            ...
            default: 'ON',
            correct: 'ON',
            correctLabel: 'ON â€” keep pump on for approach and go-around readiness',
            wrongLabel: 'Fuel pump stays ON throughout approach â€” backup pressure for go-around.',
          }] : []),
        ],
        feedback: `Final: flaps ${finalFlaps}, ${shortFinal}â€“${approach} KIAS, carb heat on${hasFuelPump ? ', fuel pump on' : ''}, stabilized. Same power, same pitch, same speed from here to the flare. If anything is wrong at 500 ft AGL â€” go around.`,
        tip: { title: 'Stabilized = safe', text: 'Stabilized approach criteria by 500 ft AGL: on centerline Â· on glidepath Â· within Â±5 kts of target Â· landing config complete Â· sink rate under 1,000 fpm. One item off = go around. No negotiating.' }
      },

      // â”€â”€ 9. GLIDEPATH â€” choice (no obvious wrong answers)
      {
        type: 'choice',
        phase: 'Final â€” Glidepath',
        ...
        options: [
          { text: 'Slightly low â€” add a small amount of power', correct: true, why: '' },
          { text: 'Slightly high â€” reduce power slightly and accept steeper descent', correct: false, why: '3R1W means LOW, not high. 1R3W = slightly high.' },
          { text: 'Dangerously low â€” go around immediately', correct: false, why: '3R1W is slightly low but recoverable with power. ALL red = dangerously low = go around.' },
          { text: 'On glidepath â€” 3 red is normal at this distance', correct: false, why: '2R2W is on glidepath. 3R1W is a one-light deviation low.' },
        ],
        ...
        tip: { title: '4 glidepath cross-checks', text: 'â‘  PAPI: 2R2W = on path  â‘¡ VSI: ~500â€“700 fpm descent at 65 KIAS is normal  â‘¢ Runway shape: if the far threshold appears to RISE toward you, you\'re too low  â‘£ Altimeter: ~300 ft AGL at 1 nm, ~150 ft AGL at Â½ nm' }
      },

      // â”€â”€ 10. FLARE â€” choice (distractors are common student errors)
      {
        type: 'choice',
        ...
        options: [
          ...
          { text: 'The runway numbers just ahead of the aircraft', correct: false, why: 'Looking close causes over-flaring â€” you flare too high and balloon, then drop.' },
        ...
        tip: { title: 'Flare sequence', text: '50 ft AGL: stop descending â€” start the flare. Smoothly raise the nose while power goes to idle (if not already there). Hold the nose off as long as possible. The mains touch, then hold back pressure to keep nose off. Don\'t "fly it onto" the runway.' }
      },

      // â”€â”€ 11. ROLLOUT CONFIG â€” config
      {
        type: 'config',
        phase: 'Rollout',
        ...
        controls: [
          {
            ...
            correctLabel: 'Hold back pressure â€” keeps weight off nosewheel, improves aerodynamic braking',
            wrongLabel: 'Hold back pressure throughout rollout to protect the nose gear and use aerodynamic drag to slow down.',
          },
          {
            id: 'flaps', label: 'Flaps', type: 'chips',
            options: [`Leave at ${finalFlaps}`, 'Retract to 0Â°', `Retract to ${baseFlaps}`],
            default: `Leave at ${finalFlaps}`,
            correct: 'Retract to 0Â°',
            correctLabel: 'Retract flaps â€” transfers lift to wheels, improves braking effectiveness',
          ...
          },
          {
            ...
            default: 'Leave ON',
            correct: 'OFF',
            correctLabel: 'OFF â€” you\'re on the ground, no longer needed',
          ...
          ...(hasFuelPump ? [{
            ...
            default: 'Leave ON',
            correct: 'OFF',
            correctLabel: 'OFF â€” electric pump no longer needed once clear of runway',
            wrongLabel: 'Turn fuel pump OFF after landing and clear of the runway.',
          }] : []),
        ],
        feedback: `Rollout: hold back pressure â†’ retract flaps â†’ carb heat off${hasFuelPump ? ' â†’ fuel pump off' : ''} â†’ brake smoothly â†’ clear the runway before running after-landing flow.`,
        tip: { title: 'After-landing flow (off the runway)', text: `Once clear: Flaps UP Â· Carb heat COLD${hasFuelPump ? ' Â· Fuel Pump OFF' : ''} Â· Transponder STBY Â· Note time Â· Taxi to parking. Do NOT run this flow while still rolling on the runway.` }
      },
    ...
    steps: [
      {
        ...
        controls: [
          { id: 'flaps', label: 'Flaps', type: 'chips', options: ['0Â°', '10Â°', '20Â°', '30Â°'], default: '10Â°', correct: '0Â°', correctLabel: '0Â° â€” flaps up for normal takeoff', wrongLabel: 'Normal takeoff uses 0Â° flaps. Flaps are only used for short-field takeoffs.' },
          { id: 'carbheat', label: 'Carb Heat', type: 'chips', options: ['OFF', 'ON'], default: 'OFF', correct: 'OFF', correctLabel: 'OFF â€” carb heat reduces power, never use on takeoff', wrongLabel: 'Carb heat must be OFF for takeoff â€” it reduces power and bypasses the air filter.' },
          { id: 'mixture', label: 'Mixture', type: 'chips', options: ['RICH', 'LEAN'], default: 'RICH', correct: 'RICH', correctLabel: 'RICH â€” full rich at sea level for max power', wrongLabel: 'Full rich for sea level takeoffs. Lean only at high-elevation airports for smooth operation.' },
        ],
        feedback: 'Normal takeoff: 0Â° flaps, carb heat off, mixture rich. Controls free and correct, transponder ALT, strobes on.',
      ...
      },
      {
        ...
        distractors: [
          { text: 'requesting takeoff clearance', why: 'No tower at KUZA â€” there\'s nobody to request clearance from. Just announce and go.' },
        ...
        feedback: 'Departure call: airport + traffic â†’ callsign â†’ action + runway â†’ airport. Same bookend structure as pattern calls.',
      ...
      },
      {
        ...
        context: 'Aircraft is rolling. You have about 8â€“10 seconds before rotation speed.',
        controls: [
          { id: 'power', label: 'Throttle', type: 'chips', options: ['Full', '1800 RPM', '2100 RPM', '75%'], default: '1800 RPM', correct: 'Full', correctLabel: 'Full throttle â€” smoothly advanced to full power', wrongLabel: 'Normal takeoff uses full throttle, smoothly applied. Partial power is only for noise abatement procedures.' },
          { id: 'check', label: 'First instrument check', type: 'chips', options: ['Airspeed alive', 'Altimeter', 'VSI', 'Heading indicator'], default: 'Altimeter', correct: 'Airspeed alive', correctLabel: 'Airspeed alive â€” confirms pitot tube is working', wrongLabel: 'First thing to verify on the roll is airspeed coming alive â€” confirms your pitot tube is unobstructed.' },
        ],
        ...
        tip: { title: 'Rudder on the roll', text: 'The C172 will want to veer left due to propeller torque and P-factor â€” use right rudder to hold centerline. This is more pronounced at full power than you expect the first few times.' }
      },
      {
        type: 'choice',
        phase: 'Rotation',
        ...
        options: [
          { text: '55 KIAS â€” rotate to ~10Â° nose up, hold for Vx or Vy', correct: true, why: '' },
          { text: '65 KIAS â€” same as approach speed to keep it simple', correct: false, why: 'Vr is 55 KIAS for the C172, not 65. Waiting until 65 wastes runway and puts you airborne at a higher speed than needed.' },
          { text: '55 KIAS â€” rotate aggressively to climb quickly', correct: false, why: 'Rotate gently to ~10Â° pitch â€” aggressive rotation can cause a tail strike or induce an accelerated stall close to the ground.' },
          { text: '45 KIAS â€” rotate early for shortest ground roll', correct: false, why: 'Rotating below Vr risks becoming airborne before the wing is generating enough lift â€” dangerous if you encounter any turbulence.' },
        ],
        feedback: 'Rotate at 55 KIAS with gentle back pressure to ~10Â° pitch attitude. The aircraft will fly itself off. Hold Vy (74 KIAS) for best rate of climb.',
        tip: { title: 'Vx vs Vy', text: 'Vx = 59 KIAS (best angle) â€” use if obstacle ahead. Vy = 74 KIAS (best rate) â€” use for normal climb. At KUZA with no obstacles, Vy is your target after rotation.' }
      },
      {
        type: 'config',
        phase: 'Initial Climb',
        ...
        controls: [
          { id: 'speed', label: 'Climb Speed', type: 'slider', min: 55, max: 100, step: 5, default: 65, unit: 'KIAS', correct: 74, tolerance: 5, correctLabel: '74 KIAS â€” Vy, best rate of climb', wrongLabel: 'Target Vy = 74 KIAS for normal climb. Vx (59) only if obstacles require steeper angle.' },
          ...
          { id: 'carbheat', label: 'Carb Heat', type: 'chips', options: ['OFF', 'ON'], default: 'OFF', correct: 'OFF', correctLabel: 'OFF â€” full power climb, carb heat not needed', wrongLabel: 'Carb heat stays OFF during climb â€” you\'re at full power and it reduces performance.' },
        ],
        feedback: 'Positive rate â€” maintain Vy (74 KIAS), full power to 500+ ft AGL, carb heat off. Track runway centerline extended, begin crosswind turn at pattern altitude.',
    ...
    steps: [
      {
        ...
        options: [
          { text: 'Clear the area with two 90Â° clearing turns, then establish entry altitude', correct: true, why: '' },
          { text: 'Reduce power immediately and add flaps', correct: false, why: 'You must clear the area first â€” slow flight puts you close to stall speed where recovery takes altitude. Check for traffic below you.' },
          { text: 'Announce on CTAF and begin entry', correct: false, why: 'Clearing turns are required to check for traffic, especially below â€” slow flight and stalls are often done without CTAF calls.' },
        ...
        feedback: 'Always do two 90Â° clearing turns before slow flight, stalls, or steep turns. Check above, below, and all around. Minimum 1,500 ft AGL recommended.',
      ...
      },
      {
        ...
        context: 'You\'re at altitude, area cleared. Enter slow flight â€” maintain altitude throughout.',
        controls: [
          { id: 'power', label: 'Power', type: 'slider', min: 600, max: 2400, step: 100, default: 2300, unit: 'RPM', correct: 1500, tolerance: 200, correctLabel: '~1500 RPM â€” reduces speed while holding altitude with back pressure', wrongLabel: 'Reduce to ~1500 RPM to begin slowing. You\'ll add more back pressure to hold altitude as speed bleeds.' },
          { id: 'flaps', label: 'Flaps', type: 'chips', options: ['0Â°', '10Â°', '20Â°', '30Â°'], default: '0Â°', correct: '10Â°', correctLabel: '10Â° â€” begin flap extension as speed allows (below 85 KIAS)', wrongLabel: 'Add flaps 10Â° once below Vfe (85 KIAS). Flaps allow slower flight while maintaining lift.' },
          { id: 'carbheat', label: 'Carb Heat', type: 'chips', options: ['OFF', 'ON'], default: 'OFF', correct: 'ON', correctLabel: 'ON â€” reduced power setting means carb ice risk', wrongLabel: 'Apply carb heat when reducing power for slow flight â€” low power settings are prime carb icing conditions.' },
        ],
        ...
        tip: { title: 'The pitch-power relationship', text: 'In slow flight you\'re operating on the "back side of the power curve" â€” more drag than normal, so adding power raises nose AND speed, and reducing power drops both. It feels reversed from cruise.' }
      },
      {
        type: 'config',
        phase: 'Maneuvering Speed',
        prompt: 'Established in slow flight. What is your target configuration?',
        context: 'ACS requires maintaining controlled flight at minimum controllable airspeed â€” just above stall with full configuration.',
        controls: [
          { id: 'flaps', label: 'Flaps', type: 'chips', options: ['0Â°', '10Â°', '20Â°', '30Â°'], default: '10Â°', correct: '30Â°', correctLabel: '30Â° (full flaps) â€” maximum lift, minimum speed', wrongLabel: 'Full flaps for slow flight â€” gives you lowest possible airspeed while maintaining controlled flight.' },
          { id: 'speed', label: 'Target Airspeed', type: 'slider', min: 40, max: 80, step: 5, default: 65, unit: 'KIAS', correct: 50, tolerance: 5, correctLabel: '~50 KIAS â€” just above stall, stall horn may intermittently sound', wrongLabel: 'Target ~50 KIAS â€” just above stall speed with full flaps. Stall horn sounding intermittently is acceptable per ACS.' },
          { id: 'rudder', label: 'Rudder', type: 'chips', options: ['Neutral', 'Right rudder', 'Left rudder'], default: 'Neutral', correct: 'Right rudder', correctLabel: 'Right rudder â€” counters left-turning tendency from torque and P-factor at high angle of attack', wrongLabel: 'Right rudder is needed to counteract torque and P-factor at low speeds/high power. Without it the ball skids left.' },
        ],
        ...
        tip: { title: 'What your examiner watches', text: 'ACS standard: maintain Â±10 kts of target airspeed, Â±100 ft altitude, Â±10Â° heading. The most common failure is ballooning altitude on flap extension or letting speed decay below stall without recovery.' }
      },
      {
        type: 'choice',
        ...
        options: [
          { text: '30Â° maximum bank â€” steep banks increase stall speed significantly', correct: true, why: '' },
          { text: '45Â° â€” same as steep turns, to practice the full range', correct: false, why: '45Â° bank in slow flight dramatically increases stall speed. At 45Â° bank, stall speed increases by ~20%. Very dangerous near MCA.' },
          { text: 'Any bank angle as long as speed is maintained', correct: false, why: 'Bank angle directly increases stall speed. You can\'t "maintain speed" through a steep bank in slow flight â€” it\'s a stall trap.' },
          { text: '20Â° maximum â€” ACS specifies this', correct: false, why: 'ACS says turns should not exceed 30Â° bank in slow flight. 20Â° is conservative but the standard is 30Â°.' },
        ],
        feedback: 'Maximum 30Â° bank in slow flight. At 30Â° bank, stall speed increases by about 7%. At 45Â°, it increases ~20% â€” easily enough to stall at your current airspeed.',
        tip: { title: 'Bank angle and stall speed', text: 'Stall speed increases with bank angle: 0Â° = 47 KIAS (C172), 30Â° = ~50 KIAS, 45Â° = ~56 KIAS, 60Â° = ~66 KIAS. In slow flight near 50 KIAS, a 60Â° bank stalls immediately.' }
      },
      {
        type: 'config',
        phase: 'Recovery',
        ...
        controls: [
          { id: 'power', label: 'Power', type: 'chips', options: ['Full then reduce', 'Idle then climb', '2300 RPM', 'Gradually increase'], default: 'Gradually increase', correct: 'Full then reduce', correctLabel: 'Full power first â€” builds speed before reducing to cruise power', wrongLabel: 'Add full power first to accelerate, then reduce to cruise power once at normal airspeed. Don\'t gradually increase â€” you need to accelerate positively.' },
          { id: 'flaps', label: 'Flaps', type: 'chips', options: ['All at once', 'Incrementally as speed builds', 'Leave until cruise', '10Â° only'], default: 'All at once', correct: 'Incrementally as speed builds', correctLabel: 'Incrementally â€” retract as airspeed increases to avoid sudden lift loss', wrongLabel: 'Retract flaps incrementally as speed builds. Retracting all at once causes a sudden lift loss and sink.' },
          { id: 'carbheat', label: 'Carb Heat', type: 'chips', options: ['Leave ON', 'OFF'], default: 'Leave ON', correct: 'OFF', correctLabel: 'OFF â€” returning to cruise power, carb heat no longer needed', wrongLabel: 'Carb heat OFF as you return to cruise power. Leaving it on reduces cruise performance.' },
        ],
        feedback: 'Recovery: full power â†’ carb heat off â†’ retract flaps incrementally as speed builds â†’ cruise power when at normal airspeed. Hold altitude throughout.',
        tip: { title: 'Common mistake', text: 'Don\'t retract flaps before you have enough speed â€” especially going from 30Â° to 20Â° to 10Â°. Each retraction step removes lift. If you retract too fast you\'ll sink or stall. Speed first, then flaps.' }
      },
    ]
  };
}

// â”€â”€ POWER OFF STALL â”€â”€
    ...
    steps: [
      {
        ...
        options: [
          { text: 'Approach to landing stall â€” minimum 1,500 ft AGL after clearing turns', correct: true, why: '' },
          { text: 'Takeoff stall â€” 3,000 ft AGL required by ACS', correct: false, why: 'Power off stall simulates an approach/landing configuration stall, not takeoff. Power on stall simulates takeoff.' },
          { text: 'Approach stall â€” no minimum altitude, just clear of clouds', correct: false, why: 'ACS recommends at least 1,500 ft AGL for stall recovery to ensure you have altitude if recovery is delayed.' },
          { text: 'Cruise stall â€” simulates unexpected speed reduction in cruise', correct: false, why: 'Power off stall is a landing configuration stall. Flaps extended, power idle â€” just like a botched approach.' },
        ],
        ...
        tip: { title: 'Oral exam question', text: '"What does a power off stall simulate?" Answer: a stall in the landing configuration â€” on approach or during the flare. It\'s the most common real-world stall scenario.' }
      },
      {
        type: 'config',
        ...
        controls: [
          { id: 'power', label: 'Power', type: 'slider', min: 600, max: 2400, step: 100, default: 2100, unit: 'RPM', correct: 1500, tolerance: 200, correctLabel: '~1500 RPM â€” simulate approach power, then reduce to idle', wrongLabel: 'Reduce to approach power (~1500 RPM) first to slow to approach speed, then reduce to idle for the stall.' },
          { id: 'flaps', label: 'Flaps', type: 'chips', options: ['0Â°', '10Â°', '20Â°', '30Â°'], default: '0Â°', correct: '30Â°', correctLabel: '30Â° â€” full flaps, landing configuration', wrongLabel: 'Full flaps simulates the landing configuration. This is where power off stalls are most likely in real life.' },
          { id: 'carbheat', label: 'Carb Heat', type: 'chips', options: ['OFF', 'ON'], default: 'OFF', correct: 'ON', correctLabel: 'ON â€” power is reduced, carb ice risk present', wrongLabel: 'Carb heat ON before reducing power to idle â€” standard procedure any time you pull power back significantly.' },
        ],
        ...
        tip: { title: 'What to expect', text: 'As you hold altitude with increasing back pressure, you\'ll feel the controls get mushy, the stall horn will sound, and then the nose will drop or a wing will drop. This is the stall â€” note the speed and feel.' }
      },
      {
        type: 'choice',
        phase: 'Stall Recognition',
        prompt: 'You\'re holding back pressure, slowing down on approach. Which comes FIRST?',
        context: 'Sequence of stall cues matters â€” the examiner will ask what you felt and when.',
        options: [
          { text: 'Stall horn â†’ buffet â†’ control mushiness â†’ nose drop', correct: false, why: 'The stall horn comes early, but buffet typically comes before or with the horn. Control mushiness happens throughout the deceleration.' },
          { text: 'Control mushiness â†’ stall horn â†’ buffet â†’ nose drop', correct: true, why: '' },
          { text: 'Nose drop â†’ stall horn â†’ buffet', correct: false, why: 'The nose drop IS the stall â€” it comes last, not first. The horn and buffet are warnings before the stall occurs.' },
          { text: 'Buffet â†’ nose drop â†’ stall horn', correct: false, why: 'The stall horn activates before the stall (it\'s a warning). It doesn\'t sound after the nose drops.' },
        ],
        feedback: 'Stall cue sequence: controls get mushy â†’ stall horn sounds â†’ airframe buffet â†’ stall (nose drops, possible wing drop). Recognizing early cues is the point of the exercise.',
        tip: { title: 'What your examiner wants', text: 'ACS requires you to recognize and announce the onset of the stall â€” not wait for the full break. Say "stall warning" when the horn sounds. The examiner is testing recognition, not just recovery.' }
      },
      {
        type: 'config',
        phase: 'Recovery',
        prompt: 'The stall has occurred â€” nose dropped. Recover with minimum altitude loss.',
        context: 'You\'re at the stall break. Every second of incorrect recovery costs altitude.',
        controls: [
          { id: 'pitch', label: 'Pitch', type: 'chips', options: ['Nose up aggressively', 'Lower nose to horizon', 'Hold current pitch', 'Lower nose slightly below horizon'], default: 'Hold current pitch', correct: 'Lower nose slightly below horizon', correctLabel: 'Lower nose slightly below horizon â€” breaks the stall angle of attack without diving', wrongLabel: 'Lower the nose slightly below the horizon to break the stall. Pushing hard nose-down dives and loses excessive altitude.' },
          { id: 'power', label: 'Power', type: 'chips', options: ['Full immediately', 'Idle â€” don\'t add power yet', '1800 RPM', 'Slowly increase'], default: 'Slowly increase', correct: 'Full immediately', correctLabel: 'Full power immediately â€” stops altitude loss and accelerates recovery', wrongLabel: 'Full power simultaneously with pitch â€” the combination minimizes altitude loss. Power alone can\'t break the stall; pitch alone wastes altitude.' },
          { id: 'flaps', label: 'Flaps', type: 'chips', options: ['Retract all immediately', 'Leave at 30Â° until flying', 'Retract to 20Â° immediately', 'Retract incrementally above 60 KIAS'], default: 'Leave at 30Â° until flying', correct: 'Retract incrementally above 60 KIAS', correctLabel: 'Retract incrementally once above 60 KIAS â€” removes drag as airspeed allows', wrongLabel: 'Don\'t retract flaps immediately â€” at the stall you have no airspeed to spare. Wait until above 60 KIAS, then retract incrementally.' },
        ],
        ...
        tip: { title: 'ACS standard', text: 'ACS requires recovery at first indication (don\'t wait for full break if possible), with minimum altitude loss. Typical power-off stall recovery uses 50â€“150 ft if done promptly. Delay costs 300+ ft.' }
      },
    ]
  };
}

// â”€â”€ POWER ON STALL â”€â”€
    ...
    steps: [
      {
        ...
        options: [
          { text: 'Takeoff/departure stall â€” torque and P-factor make left yaw/roll more likely', correct: true, why: '' },
          { text: 'Approach stall with engine â€” same as power off but with power added', correct: false, why: 'Power on stall simulates takeoff and departure, not approach. Full power changes the aerodynamics and control forces significantly.' },
          { text: 'Cruise stall â€” simulates speed loss in level flight', correct: false, why: 'Power on stall is specifically a takeoff/departure configuration â€” high power, flaps up or minimal, climbing attitude.' },
          { text: 'Go-around stall â€” same procedure as power off but faster', correct: false, why: 'While a go-around stall is related, power on stall specifically simulates the takeoff departure scenario with full power applied.' },
        ],
        ...
        tip: { title: 'Why it\'s different', text: 'At full power and high angle of attack, P-factor and torque are at maximum. The left-turning tendency is very strong â€” expect the left wing to drop at the stall break if you\'re not holding right rudder.' }
      },
      {
        type: 'config',
        phase: 'Entry Configuration',
        ...
        controls: [
          { id: 'flaps', label: 'Flaps', type: 'chips', options: ['0Â°', '10Â°', '20Â°', '30Â°'], default: '10Â°', correct: '0Â°', correctLabel: '0Â° â€” takeoff configuration, flaps up', wrongLabel: 'Power on stall uses 0Â° flaps â€” takeoff configuration. Some instructors allow 10Â° for short-field, but 0Â° is standard.' },
          { id: 'power', label: 'Power', type: 'chips', options: ['Full', '1800 RPM', '2100 RPM', '1500 RPM'], default: '2100 RPM', correct: 'Full', correctLabel: 'Full power â€” simulates takeoff power setting', wrongLabel: 'Full power for power on stall â€” that\'s the defining characteristic. Simulates full takeoff power.' },
          { id: 'pitch', label: 'Pitch Attitude', type: 'chips', options: ['Level', '10Â° nose up', '15â€“20Â° nose up', '5Â° nose up'], default: '10Â° nose up', correct: '15â€“20Â° nose up', correctLabel: '15â€“20Â° nose up â€” aggressive climb attitude induces stall rapidly', wrongLabel: 'Pitch to 15â€“20Â° nose up at full power to bring on the stall. This simulates over-rotating on takeoff.' },
        ],
        feedback: 'Power on stall entry: full power, 0Â° flaps, pitch aggressively to 15â€“20Â° nose up. Hold heading with right rudder â€” the torque will want to yaw left hard.',
      ...
      },
      {
        ...
        context: 'Full power, high pitch, approaching stall â€” what happens at the break?',
        options: [
          { text: 'Left yaw and left wing drop â€” from torque and P-factor', correct: true, why: '' },
          { text: 'Right wing drop â€” propeller slipstream pushes right side down', correct: false, why: 'Torque and P-factor both create left-turning tendencies. Slipstream spirals around the fuselage but the net effect at high power/AOA is left yaw.' },
          { text: 'Straight nose drop â€” same as power off stall', correct: false, why: 'Power on stalls rarely break straight ahead â€” the torque and P-factor at full power usually produce a left roll/yaw at the break.' },
        ...
        feedback: 'Power on stalls typically break to the left â€” left wing drops, left yaw â€” due to torque and P-factor. Right rudder throughout the entry reduces the severity of this break.',
        tip: { title: 'What to say', text: 'Announce "stall" at the break. Your examiner wants to hear you recognize it. Recovery priority: wings level â†’ nose down slightly â†’ maintain full power â†’ climb away.' }
      },
      {
        type: 'config',
        phase: 'Recovery',
        prompt: 'Left wing drops at the stall break. Recover.',
        context: 'Full power stall break with left roll tendency. Sequence matters â€” wrong order can aggravate the stall.',
        controls: [
          { id: 'rudder', label: 'First control input', type: 'chips', options: ['Right aileron to level', 'Right rudder to stop yaw', 'Left aileron', 'Elevator forward'], default: 'Right aileron to level', correct: 'Right rudder to stop yaw', correctLabel: 'Right rudder first â€” stops the yaw before it becomes a spin entry', wrongLabel: 'Right RUDDER first to stop the yaw. Using aileron first in a stall can aggravate the roll (aileron drag stalls the dropping wing further).' },
          { id: 'pitch', label: 'Pitch input', type: 'chips', options: ['Aggressive nose down', 'Slight nose down â€” break AOA', 'Hold pitch', 'Nose up to stop sink'], default: 'Hold pitch', correct: 'Slight nose down â€” break AOA', correctLabel: 'Slight nose down â€” breaks angle of attack without excessive altitude loss', wrongLabel: 'Lower nose just enough to break the stall AOA. Aggressive push wastes altitude unnecessarily.' },
          { id: 'power', label: 'Power', type: 'chips', options: ['Keep full power', 'Reduce to 1800 RPM', 'Idle', 'Reduce then add'], default: 'Keep full power', correct: 'Keep full power', correctLabel: 'Keep full power â€” you already have it in, use it to accelerate recovery', wrongLabel: 'Power on stall recovery keeps full power â€” it\'s already helping you. Reducing power wastes the energy you have.' },
        ],
        feedback: 'Recovery sequence: right rudder to stop yaw â†’ slight nose down to break stall â†’ wings level with coordinated aileron â†’ climb at Vy. Full power maintained throughout.',
        tip: { title: 'Spin awareness', text: 'An uncoordinated stall break â€” ball to the left, left wing dropping â€” is a spin entry. Right rudder prevents this. If a wing drops at the break and you apply opposite aileron before rudder, you can worsen the roll. Rudder first, always.' }
      },
    ]
  };
}
...
function renderProcStep() {
  ...
  if (idx >= steps.length) {
    ...
    document.getElementById('proc-step-content').innerHTML = `
      <div class="proc-complete">
        <div class="proc-complete-icon">ðŸ›¬</div>
        <div class="proc-complete-title">Procedure Complete</div>
        <div class="proc-complete-sub">Pattern Entry &amp; Landing Â· ${procState.airport.name || 'Your Field'}</div>
  ...
  const tipHtml = step.tip ? `<div class="proc-tip-box"><div class="proc-tip-label">ðŸ’¡ ${step.tip.title}</div>${step.tip.text}</div>` : '';
  ...
  document.getElementById('proc-step-content').innerHTML = `
    ...
    <button class="proc-next-btn" id="proc-next-btn" onclick="nextProcStep()">
      ${idx + 1 < steps.length ? 'Next Step â†’' : 'Finish âœ“'}
    </button>`;
...
function checkConfigStep() {
  ...
  step.controls.forEach(ctrl => {
    ...
    if (ctrl.type === 'slider') {
      ...
      const rangeStr = `${lo}â€“${hi} ${ctrl.unit}`;
      row.classList.add(isCorrect ? 'correct' : 'wrong');
      result.textContent = isCorrect
        ? `âœ“ ${ctrl.correctLabel} (accepted range: ${rangeStr})`
        : `âœ— ${ctrl.wrongLabel} (accepted range: ${rangeStr})`;
    } else {
      ...
      row.classList.add(isCorrect ? 'correct' : 'wrong');
      result.textContent = isCorrect ? `âœ“ ${ctrl.correctLabel}` : `âœ— ${ctrl.wrongLabel}`;
    }
  ...
  if (allCorrect) {
    fb.textContent = 'âœ“ ' + step.feedback;
    document.getElementById('proc-next-btn').classList.add('show');
  } else {
    // Hide wrong labels â€” don't reveal answers yet
    step.controls.forEach(ctrl => {
      const result = document.getElementById('result-' + ctrl.id);
      const row = document.getElementById('row-' + ctrl.id);
      if (row.classList.contains('wrong')) result.textContent = 'âœ— Not quite';
    });
    fb.innerHTML = `âœ— One or more controls need adjustment.<br><br>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:4px">
        <button class="btn btn-sm" onclick="retryConfigStep()">â†© Try Again</button>
  ...
  step.controls.forEach(ctrl => {
    ...
    if (ctrl.type === 'slider') {
      ...
      result.textContent = row.classList.contains('correct')
        ? `âœ“ ${ctrl.correctLabel} (accepted: ${lo}â€“${hi} ${ctrl.unit})`
        : `âœ— ${ctrl.wrongLabel} (accepted: ${lo}â€“${hi} ${ctrl.unit})`;
    } else {
      result.textContent = row.classList.contains('correct')
        ? `âœ“ ${ctrl.correctLabel}`
        : `âœ— ${ctrl.wrongLabel}`;
    }
  });
...
function renderRadioStep(step) {
  ...
  return `
    ...
    <div class="word-bank-hint">tap a word to add Â· tap again to remove</div>
...
function checkProcRadio() {
  ...
  if (isCorrect) {
    ...
    if (usedDistractors.length > 0) {
      msg += '\n\nâš  Trap chips you included:\n' +
        usedDistractors.map(d => `â€¢ "${d.text}" â€” ${d.why}`).join('\n');
    }
    fb.innerHTML = msg.replace(/\n/g, '<br>') +
      `<br><br><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:4px">
        <button class="btn btn-sm" onclick="clearProcRadio()">â†© Try Again</button>
...
function revealProcRadio() {
  ...
  const fb = document.getElementById('proc-feedback');
  fb.className = 'proc-feedback show wrong';
  fb.innerHTML = `âœ— ${step.feedback}<br><br><strong>Ideal:</strong> ${step.ideal}`;
  document.getElementById('proc-next-btn').classList.add('show');
}
...
function answerProcStep(btn) {
  ...
  fb.textContent = (isCorrect ? 'âœ“ ' : 'âœ— ') + step.feedback;
  document.getElementById('proc-next-btn').classList.add('show');
}
...
function renderSeqRecall() {
  ...
  let html = `
    ...
    <div class="seq-slot-area">
      <div class="seq-slot-label">// Your sequence â€” drag to reorder</div>
    ...
    <div class="seq-pool-label seq-slot-label">// Item pool â€” drag or tap to place</div>
            ...
            <div class="seq-slot-content">
              ${isCorrect
                ...
                : `<span style="color:var(--red)">${userItem ? userItem.action : '?'}</span>
                   <span style="font-size:10px;color:var(--text-dim)"> â†’ correct: ${item.action}</span>`}
            </div>
          </div>`;
        }).join('')}
      </div>
...
function seqPoolClick(i) {
  ...
  const inSlots = new Set(seqState.order.filter(x => x !== -1));
  if (inSlots.has(item.origIdx)) return; // already placed â€” ignore
...
function seqSlotClick(slotIdx) {
  ...
  if (origIdx !== -1) {
    // Slot has an item â€” remove it back to pool
...
function setRadioInputMode(mode) {
  ...
  hdr.textContent = mode === 'chips'
    ? 'Build your call â€” tap words in order'
...
function checkSpeechCall() {
  ...
  hdr.textContent = isGood
    ? `âœ“ Good call â€” ${Math.round(result.score * 100)}% match`
    : `âœ— ${Math.round(result.score * 100)}% match â€” review below`;
  ...
  document.getElementById('feedback-ideal').innerHTML =
    `<div style="margin-bottom:8px;font-size:11px;color:#9ab8d0">Ideal call â€” word match:</div>
  ...
  document.getElementById('feedback-note').innerHTML = note +
    (!isGood ? `<br><br><div style="display:flex;gap:8px;margin-top:8px">
      <button class="btn btn-sm" onclick="clearRadioCallActive()">â†© Try Again</button>
...
function newATIS() {
  ...
  document.getElementById('atis-transcript').innerHTML = '<span class="atis-trans-label">Tap Play to hear the ATIS â€” then fill in what you heard</span>';
  document.getElementById('atis-play-btn').textContent = 'â–¶ Play';
...
function playATIS() {
  ...
  if (!synth) {
    document.getElementById('atis-transcript').textContent = 'Text-to-speech not available â€” read the transcript below.';
  ...
  document.getElementById('atis-play-btn').textContent = 'â¹ Stop';
  ...
  utt.onend = () => {
    ...
    document.getElementById('atis-play-btn').textContent = 'â–¶ Play Again';
    document.getElementById('atis-transcript').innerHTML =
      `<span style="color:#9ab8d0;font-size:11px">ATIS read â€” fill in what you heard. You can play again if needed.</span>`;
  };
...
function checkATIS() {
  ...
  const results = d.fields.map(f => {
    ...
    if (f.key === 'altimeter') {
      ...
      // Accept directionÂ±10 and speedÂ±2
  ...
  resultsEl.innerHTML = `
    <div class="atis-score-banner">
      ...
      <div class="atis-score-sub">${correct} of ${d.fields.length} correct Â· ${pct}%</div>
    </div>
    <div style="background:#111f30;border:1px solid #2a4060;border-radius:4px;padding:14px;margin-bottom:14px">
      ${results.map(r => `
        <div class="atis-result-row">
          ...
          <div class="atis-result-yours ${r.isCorrect ? 'correct' : 'wrong'}">${r.userVal || 'â€”'}</div>
          ${!r.isCorrect ? `<div class="atis-result-correct">â†’ ${r.answer}</div>` : ''}
        </div>
      `).join('')}
    </div>
    ...
    <div style="display:flex;gap:8px">
      ...
      <button class="btn" onclick="playATISAgain()">â–¶ Play Again</button>
    </div>`;
}
