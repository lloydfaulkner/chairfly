...
function scoreSpeechCall(spoken, ideal, words, optionalWords = []) {
  const normalize = s => {
    ...
    // teens → digits (needed for altitudes like "eighteen hundred" → 1800)
    ...
    // tens words → digits (for "thirty five hundred" etc.)
    ...
    // collapse "N thousand M hundred" → N*1000+M*100 ("three thousand five hundred" → 3500)
    t = t.replace(/\b(\d+) thousand (\d+) hundred\b/g, (m,a,b) => String(parseInt(a)*1000+parseInt(b)*100));
    // collapse "N thousand" → N*1000
    t = t.replace(/\b(\d+) thousand\b/g, (m,n) => String(parseInt(n)*1000));
    // collapse "N hundred" → N*100 ("35 hundred" → 3500, "eighteen hundred" → 1800)
  ...
  const wordResults = keyWords.map(kw => {
    ...
    // multi-digit keyWord (e.g. "3500"): match if digit sequence appears in spoken digit stream
    if (/^\d{2,}$/.test(kw) && spokenDigits.includes(kw)) return { word: kw, status: 'match' };
    // single digit keyWord (e.g. "4" from tail number): match if it appears in a spoken multi-digit token
    // (handles speech recognizer returning "4521" instead of "four five two one")
