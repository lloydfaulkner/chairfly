const ALL_AIRCRAFT = {
  ...
  runup: {
    ...
    items: [
      ...
      { action: 'Primer', value: 'IN & LOCKED', note: 'Confirm locked — vibration can unlatch it', bucket: 'free',
      ...
      { action: 'Avionics', value: 'ON & SET', note: 'Radios, transponder set to ALT, GPS programmed', bucket: 'free',
      ...
      { action: 'Squawk Code', value: 'SET & VERIFY', note: 'Enter code from ATC clearance; confirm ALT mode', bucket: 'free',
      ...
      { action: 'Doors & Windows', value: 'CLOSED & LATCHED', note: 'Secure before takeoff', bucket: 'free',
  ...
  cherokee140: {
    ...
    checklists: {
      ...
      beforestart: {
        label: 'Before Start',
        items: [
          { action: 'Seat Track / Back', value: 'LOCK', note: 'Adjust and lock — both occupants', why: 'Unlocked seat rails can cause the seat to slide aft on rotation — you\'d lose control with no way to reach the yoke.', tip: 'Push seat back hard after adjusting. No movement = locked.', zone: 'seats', bucket: 'gate' },
          { action: 'Avionics', value: 'OFF', note: 'Protect from start voltage spike', why: 'Engine start causes a momentary voltage spike that can damage radios and GPS. Turn all avionics off before master.', tip: '"Avionics off first, avionics on last" — every aircraft, every time.', zone: 'avionics', bucket: 'free' },
          { action: 'Autopilot', value: 'OFF', note: 'Disconnect before start', why: 'Autopilot servos engaged during start can cause unexpected control inputs when the electrical system surges.', tip: 'Short toggle — verify the AP annunciator is out.', zone: 'avionics', bucket: 'free' },
          { action: 'Carb Heat', value: 'OFF (COLD)', note: 'Cold for start — on only if icing suspected in flight', why: 'Carb heat bypasses the air filter and reduces power. Never use it on the ground unless troubleshooting icing.', tip: 'The Cherokee Lycoming O-320 is particularly susceptible to carb ice at low power — always check carb heat in flight.', zone: 'carbheat', bucket: 'free' },
          { action: 'Mixture', value: 'FULL RICH', note: 'Rich for start at sea level', why: 'Rich mixture ensures adequate fuel for cold start. Lean only at high density altitude airports.', tip: 'Red knob full IN = rich.', zone: 'mixture', bucket: 'free' },
          { action: 'Throttle', value: 'SLIGHT', note: '1/4 to 1/2 inch open', why: 'Too closed and the engine won\'t catch. Too open and it may race dangerously after starting.', tip: 'Two finger widths from fully closed.', zone: 'throttle', bucket: 'free' },
          { action: 'Fuel Selector', value: 'PROPER TANK', note: 'Fuller tank for start', why: 'The Cherokee has no BOTH — you must select L or R. The fuller tank reduces the chance of running a tank dry at a bad time.', tip: 'Look at the selector, not just feel it. The detent clicks are similar for L and R.', zone: 'fuel', bucket: 'free' },
          { action: 'Brakes', value: 'SET', note: 'Hold firmly during start', why: 'Engine start generates prop blast. Holding brakes keeps the aircraft stationary on the ramp.', tip: 'Press both toe brakes and hold. Release only when ready to taxi.', zone: 'tires', bucket: 'free' },
        ]
      },
      start: {
        label: 'Engine Start',
        items: [
          { action: 'Beacon', value: 'ON', note: 'Signal others — engine about to start', why: 'The beacon is the universal "prop is turning" warning. Always on before the starter engages.', tip: 'Beacon on = engine running or starting. Last thing off after shutdown.', zone: 'beacon', bucket: 'gate' },
          { action: 'Master', value: 'ON', note: 'Both ALT and BAT', why: 'Powers the electrical system. Some pilots turn BAT on first to check voltage, then ALT.', tip: 'Left side = ALT, right side = BAT on the split rocker.', zone: 'master', bucket: 'ordered' },
          { action: 'Fuel Pump', value: 'ON', note: 'Electric boost pump on for start', why: 'The Cherokee has an electric fuel pump to prime the fuel-injected or carbureted system and ensure fuel pressure before the engine-driven pump takes over. On for start, off after engine running.', tip: 'Check fuel pressure rises after pump ON. Should show green within seconds.', zone: 'throttle', bucket: 'ordered' },
          { action: 'Prime', value: 'AS REQUIRED', note: 'Cold engine: 2–4 strokes; hot start: none', why: 'Cold engines need raw fuel in the intake. Hot engines already have residual fuel — over-priming floods it.', tip: 'Hot start: skip prime, set mixture idle-cutoff, crank until it catches, then advance mixture quickly.', zone: 'primer', bucket: 'free' },
          { action: 'Mags', value: 'START', note: 'Both mags — START position', why: 'START engages the starter motor. Release immediately when the engine catches.', tip: 'Don\'t crank more than 10 seconds. Rest 30 seconds between attempts to protect the starter.', zone: 'ignition', bucket: 'ordered' },
          { action: 'Fuel Pump', value: 'OFF (after start)', note: 'Off once engine running smoothly', why: 'Running the electric pump continuously can flood the carburetor or cause vapor lock in fuel-injected models. Off after engine-driven pump takes over.', tip: 'Watch fuel pressure — it should hold steady after turning pump off. If it drops: investigate.', zone: 'throttle', bucket: 'free' },
          { action: 'Oil Pressure', value: 'CHECK (30s)', note: 'Should rise within 30 seconds', why: 'No oil pressure means oil isn\'t circulating. Continued running causes rapid engine damage.', tip: 'Eyes on the oil pressure gauge immediately after start. Green within 30s or shut down.', zone: 'sixpack', bucket: 'free' },
        ]
      },
      runup: {
        label: 'Runup',
        items: [
          { action: 'Engine Warm-up', value: '1000 RPM', note: 'Allow oil temp to rise before runup', why: 'Cold oil doesn\'t lubricate as well. Wait for oil temperature to show movement before runup checks.', tip: 'On cold days this may take several minutes. Don\'t rush it.', zone: 'sixpack', bucket: 'gate' },
          { action: 'Throttle', value: '2000 RPM', note: 'Set for runup checks', why: 'The Cherokee Lycoming O-320 runup is typically done at 2000 RPM — slightly higher than the C172\'s 1700-1800 RPM setting.', tip: 'Set smoothly. Rapid throttle advances can cause engine stress when cold.', zone: 'throttle', bucket: 'ordered' },
          { action: 'Magnetos', value: 'CHECK L/R (R-L-Both)', note: 'Max RPM drop per mag check', why: 'Each magneto is tested independently. A large RPM drop indicates fouled plugs or mag issues.', tip: 'BOTH → R (note drop) → BOTH → L (note drop) → BOTH. Max ~125 RPM drop, max 50 RPM difference between mags.', zone: 'ignition', bucket: 'ordered' },
          { action: 'Carb Heat', value: 'ON — TEST', note: 'Slight RPM drop expected; return to COLD', why: 'Warm unfiltered air reduces power slightly — a small RPM drop is normal and expected. Return to COLD before takeoff.', tip: 'If RPM initially drops then rises above original = you had ice. Normal drop then back = no ice.', zone: 'carbheat', bucket: 'ordered' },
          { action: 'Vacuum', value: 'CHECK', note: 'Should read ~5 in. Hg in green arc', why: 'Vacuum drives the attitude indicator and directional gyro. Low vacuum means unreliable gyro instruments.', tip: 'Green arc on the gauge. Any reading below = investigate before flight in IMC.', zone: 'sixpack', bucket: 'free' },
          { action: 'Amps / Volts', value: 'CHECK', note: 'Ammeter should show charge', why: 'Confirms the alternator is charging the battery. Discharge at runup = possible alternator or regulator issue.', tip: 'Ammeter needle in the + (positive/charge) side. Zero or negative = problem.', zone: 'sixpack', bucket: 'free' },
          { action: 'Oil Pressure', value: 'GREEN', note: 'In normal operating range', why: 'Oil pressure should be fully stabilized by runup. Any fluctuation or below-green reading = abort.', zone: 'sixpack', tip: 'If it was good at startup but has dropped at runup — something changed. Don\'t depart.', bucket: 'free' },
          { action: 'Oil Temperature', value: 'GREEN', note: 'Warm enough to proceed', why: 'Running a full-power takeoff with cold oil stresses engine internals. Oil temp in green = ready.', tip: 'Wait for the needle to enter the green arc — not just show movement.', zone: 'sixpack', bucket: 'free' },
          { action: 'Throttle', value: 'IDLE — CHECK CLOSED', note: 'Verify idle; return to 1000 RPM for taxi', why: 'Checks that the throttle returns to proper idle and won\'t stick.', tip: 'Idle should be smooth at ~600-700 RPM. Rough idle = possible fouled plug or carb issue.', zone: 'throttle', bucket: 'free' },
          { action: 'Friction Lock', value: 'AS DESIRED', note: 'Set throttle friction', why: 'The Cherokee has a friction lock on the throttle to prevent creep. Set to hold position without requiring constant hand pressure.', tip: 'Tighten until throttle stays where you set it but can still be moved deliberately.', zone: 'throttle', bucket: 'free' },
          { action: 'Flight Controls', value: 'FREE & CORRECT', note: 'Full deflection each direction', why: 'Final confirmation that nothing is blocking controls before takeoff. Full deflection only — partial doesn\'t count.', tip: 'Yoke right: right aileron UP. Back: elevator UP. Rudder left: left rudder.', zone: 'controls', bucket: 'free' },
          { action: 'Trim', value: 'SET TAKEOFF', note: 'Elevator trim to takeoff range', why: 'Correct trim reduces stick force at rotation. Wrong trim = fighting the aircraft at the worst moment.', tip: 'Trim wheel is on the ceiling between the seats in most Cherokees — easy to miss.', zone: 'trim', bucket: 'free' },
          { action: 'Fuel Selector', value: 'PROPER TANK', note: 'Fuller tank for takeoff', why: 'Takeoff is the most critical phase. Use the fuller tank so you have maximum fuel available if a go-around is needed immediately.', tip: 'Look at the selector — don\'t just feel the position.', zone: 'fuel', bucket: 'free' },
          { action: 'Primer', value: 'IN & LOCKED', note: 'Confirm locked before takeoff', why: 'Vibration can unlatch an improperly locked primer, causing lean mixture and rough running at full power.', tip: 'Tug it outward — it should not move at all.', zone: 'primer', bucket: 'free' },
          { action: 'Doors & Windows', value: 'CLOSED & LATCHED', note: 'Secure all before departure', why: 'A Cherokee door opening in flight creates noise and distraction. The Cherokee\'s door design makes it prone to appearing closed when it isn\'t fully latched.', tip: 'Lift the handle and push the door firmly inward. Listen for the latch click.', zone: 'seats', bucket: 'free' },
        ]
      },
      landing: {
        label: 'Before Landing',
        items: [
          { action: 'Fuel Selector', value: 'PROPER TANK', note: 'Fullest tank for approach and go-around', why: 'Engine failure on approach due to fuel exhaustion on the selected tank is a known accident cause. Use the fuller tank.', tip: 'Part of GUMPS on downwind — G = Gas (proper tank).', tipType: 'acronym', acronym: 'GUMPS', acronymDef: 'Gas (proper tank) · Undercarriage · Mixture (rich) · Prop · Seatbelts', zone: 'fuel', bucket: 'gate' },
          { action: 'Fuel Pump', value: 'ON', note: 'Electric pump on for approach', why: 'The electric boost pump provides backup fuel pressure in case the engine-driven pump falters at low power settings during approach. Critical for go-around power availability.', tip: 'On before you start the approach. Off only after landing and clear of the runway.', zone: 'throttle', bucket: 'ordered' },
          { action: 'Mixture', value: 'BEST POWER', note: 'Enrich for go-around capability', why: 'A lean mixture at full throttle during a go-around can cause the engine to run rough or lose power at the worst moment. Enrich before approach.', tip: 'At low altitudes: full rich. At higher elevations: best power mixture (slightly leaned).', zone: 'mixture', bucket: 'ordered' },
          { action: 'Carb Heat', value: 'AS REQUIRED', note: 'On if icing conditions suspected', why: 'Carb ice forms readily at low power — prime approach conditions. Apply carb heat if temps are 20-70°F and there\'s visible moisture.', tip: 'Remember to go CARB HEAT OFF for go-around — you need full power without restriction.', zone: 'carbheat', bucket: 'free' },
          { action: 'Seats & Belts', value: 'SECURE', note: 'Both occupants — shoulder harness on', why: 'Landing is the highest-risk phase for injury. Shoulder harnesses dramatically reduce injury in hard landings. Check your passenger too.', tip: 'Announce it out loud: "Belts — mine\'s locked." Then look at your passenger.', zone: 'seats', bucket: 'free' },
          { action: 'Flaps', value: '10°–25° AS REQ.', note: 'Progressive — based on conditions', why: 'Unlike the C172\'s fixed flap schedule, the Cherokee gives you flexibility. 10° for short field initial, 25° for normal full flap landing. 40° is the maximum.', tip: '25° flaps for normal full-stop. Below Vfe (101 KIAS) before extending.', zone: 'flaps', bucket: 'free' },
          { action: 'Airspeed (final)', value: '70 KIAS / 59 SHORT', note: 'Approach 70, short final 59 KIAS', why: 'The Cherokee is heavier and stalls at a higher speed than it appears. 70 KIAS gives adequate margin. Short final 59 KIAS is the published approach speed with flaps.', tip: 'Add half the gust factor in gusty winds. If gusting 10 knots, fly final at 74-75 KIAS.', zone: 'sixpack', bucket: 'free' },
          { action: 'Go-Around', value: 'BRIEF & READY', note: 'Unstabilized at 500 ft AGL = go around', why: 'The Cherokee is heavier and slower to accelerate than a C172. A go-around decision made late is a go-around that might not succeed. Decide early.', tip: 'Go-around: Full power → Carb heat OFF → Positive climb → Flaps 25° → Flaps 0° above 70 KIAS.', zone: 'throttle', bucket: 'free' },
        ]
      }
    },
    emergencies: [
